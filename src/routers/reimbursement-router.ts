...
import { findReimbursementById, findReimbursementByStatusId, saveOneReimbursement} from '../services/reimbursement-service'
...
// a post request to the root of a resource will make one new of that resource
 reimbursementRouter.post('', authenticationFactory(['Everyone']), async (req,res)=>{
     let {reimbursementId, author, amount, 
     ...
     if(reimbursementId && author && amount &&
         ...
         res.sendStatus(201).json(newReimbursement)// if I don't need to seend a body
     } else {
